...
export const getShopImageAndName = async () => {
    const response = await publicApi.get("/api/v2/shop-profile/shop-name/shop-image");
    return response.data;
}
...
export const getTopSellingProduct = async ({range, page, size} : TopSellingProductRequest) => {
...
export const getAllProductsStatus = async ({page, size}:{page:number, size:number}) => {
    const response = await api.get(`/api/v2/product/get-statuses?page=${page}&size=${size}`);
    return response.data;
}