...
import { useProductStockStatusUpdate } from "./useStockStatusWebSocket";
...
export function useStockStatus ({page, size}:{page: number, size:number}) {
    ...
    useEffect(() => {
        ...
        queryClient.prefetchQuery({
            ...
            queryFn: () => getAllProductsStatus({page: nextPage, size}).then((res) => res.data),
        })
    }, [page, totalPages, queryClient, size]);
    ...
    useProductStockStatusUpdate({
        onStockStatusUpdate: (updatedProduct: ProductStock) => {
            queryClient.setQueryData(queryKey, (oldData: StockStatusResponse) => {
                ...
                return {
                    ...oldData,
                    products: oldData.products.map((p: ProductStock) => p.id === updatedProduct.id ? updatedProduct : p),
                }
            })
        }
    });
